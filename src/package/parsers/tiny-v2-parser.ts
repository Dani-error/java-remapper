import type { ClassMapping, FieldMapping, MappingSet, MethodMapping, ParameterMapping } from "../types";
    ...
    parse(content: string[]): MappingSet {
        ...
        for (let x = start; x < content.length; x++) {
            ...
            switch (parts[0]) {
                ...
                case "f": // Field
                    if (parts.length !== nameCount + 2 || stack[stack.length - 1] !== TinyV2State.CLASS)
                ...
                case "p": // Parameter
                    if (parts.length !== nameCount + 2 || stack[stack.length - 1] !== TinyV2State.METHOD)
                        throw tiny2Exception(x, line);
